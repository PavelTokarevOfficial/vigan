...
Внизу главной страницы находится библиотека файлов из `downloads/`: видео можно просмотреть прямо в браузере. Кнопка «Сделать субтитры и видео» запускает локальную транскрибацию через `whisper.cpp`, сохраняет один `.srt` с таймингами и создаёт рядом mp4 с вшитыми субтитрами. Имя нового видео начинается с `width-sub-`. Подробнее: [docs/whisper-subtitles.md](docs/whisper-subtitles.md).
