...
<template>
  <section>
    ...
    <div
      v-if="!videos.length"
      class="mt-6 rounded border border-dashed p-8 text-slate-500"
    >
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      <article
        v-for="v in videos"
        :key="v.storageKey"
        class="mt-4 rounded bg-white p-4 shadow-sm"
      >
        ...
        <div class="mt-2 flex flex-wrap justify-between gap-2">
          ...
          <span class="flex gap-3">
            <a :href="v.twitchUrl" target="_blank" class="text-violet-700"
              >Оригинал</a
            ><a :href="v.url" target="_blank" class="text-violet-700">Открыть</a
            ><a :href="v.url" download class="text-violet-700">Скачать</a></span
          >
        </div>
      </article>
    </div>
